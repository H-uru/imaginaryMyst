...
bool imAVILayer::read(imStream* stream)
{
    ...
    imLog("DEBUG: [SDB] [imAVILayer] (Filename) '{}'", m_filename);

    if ((m_animFlags & 0x8) != 0) {
        imLog("DEBUG: [SDB] [imAVILayer] (+9C) {}", stream->read32());
        imLog("DEBUG: [SDB] [imAVILayer] (+A0) {}", stream->read32());
    }
    return true;
}
...
bool imQTLayer::read(imStream* stream)
{
    ...
    size_t len = stream->read32();
    m_filename = stream->readString(len);
    imLog("DEBUG: [SDB] [imQTLayer] (Filename) '{}'", m_filename);
    return true;
}
...
bool imBinkLayer::read(imStream* stream)
{
    ...
    size_t len = stream->read32();
    m_filename = stream->readString(len);
    imLog("DEBUG: [SDB] [imBinkLayer] (Filename) '{}'", m_filename);
    return true;
}
...
bool imWaterLayer::read(imStream* stream)
{
    ...
    imLog("DEBUG: [SDB] [imWaterLayer] (+EC) {}", stream->read32());
    imLog("DEBUG: [SDB] [imWaterLayer] (+F4) {}", stream->read32());

    size_t count = stream->read32();
    for (size_t i=0; i<count; i++) {
        imLog("DEBUG: [SDB] [WaterEffect] (+0) {}", stream->read32());
        imLog("DEBUG: [SDB] [WaterEffect] (+4) {}", stream->read32());
        imLog("DEBUG: [SDB] [WaterEffect] (+8) {}", stream->read32());
        imLog("DEBUG: [SDB] [WaterEffect] (+0C) {}", stream->read32());
        imLog("DEBUG: [SDB] [WaterEffect] (+10) {}", stream->read32());
        imLog("DEBUG: [SDB] [WaterEffect] (+14) {}", stream->read32());
    }
    return true;
}
